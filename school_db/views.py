...
def example_solution(request):
    ...
    for item in results:
        print(
            f'Course Name: {item.name} Credits: {item.credits} Instructor: {item.instructor.first_name}')
...
def problem_two(request):
    all_instructors = Instructor.objects.filter(hire_date__iso_year__lt="2010")
...
def problem_three(request):
    ...
    courses_by_instructor_id = Course.objects.filter(instructor__id=2)
    print(courses_by_instructor_id)
    for instructor in courses_by_instructor_id:
...
def problem_four(request):
    ...
    print(f'Student Count: {student_count} Instructor Count: {instructor_count} Course Count: {course_count}')
    #course_count = Course.objects.filter(course_count='Course').count()
    #for Course in course_count:
      #  print(Course)
    #instructor_count = Instructor.objects.filter(
...
def problem_five(request):
    New_student= Student.objects.create(first_name= "Beret Celeste", last_name= "Owens", year=1983, gpa=3.88)
    print(f' ID: {New_student.pk} Full Name: {New_student.first_name} {New_student.last_name} Year: {New_student.year} GPA: {New_student.gpa} ')
...
def problem_six(request):
    student_id = 15
    Student.objects.filter(pk=student_id).update(gpa=3.98)
    student = Student.objects.get(pk=student_id)
    print(f' ID: {student.pk} Full Name: {student.first_name} {student.last_name} Year: {student.year} GPA: {student.gpa} ')
...
def problem_seven(request):
    ...
    # Make sure to set this equal to the primary key of the row you just created!
    student_id = 15
    Student.objects.filter(student= student_id).delete(student_id=15)
    #try:
     #   student = Student.objects.get(pk=student_id)
